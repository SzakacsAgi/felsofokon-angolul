...
import { isErrorOccurred, extractInnerText, convertTitleToLink } from "../store/utils/content-displayer-util";
...
export default function Services({ pageContent }) {
    ...
    return <>
        ...
        {servicesSectionContent.data.map(service => {
            return <section id={convertTitleToLink(extractInnerText(service.serviceTitle))}>
                <SectionTitle title={extractInnerText(service.serviceTitle)} key={convertTitleToLink(extractInnerText(service.serviceTitle))} />
